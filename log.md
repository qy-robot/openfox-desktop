...
- 2026-09-27T01:17:02+08:00 | Codex | `feat/powershell-robot-terminal` 的入口页/纯显示终端修正已提交并推送，功能提交 `241479567e`：两个目标选项位于右侧栏开始页，终端内部不再出现输入框、快捷键、SSH 表单或模式切换，只保留输出与按需执行确认；Beta 源码窗口正在运行，等待用户验收。

- 2026-09-27T01:14:34+08:00 | Codex | `feat/powershell-robot-terminal` 按用户澄清恢复终端职责边界：右上角“终端”先打开右侧栏原生“开始”页，由该入口页提供“进入本机设备终端 / 进入机器人终端”两个选项；终端主体只显示连接状态与运行输出，不再渲染手动命令框、快捷按键或 SSH 配置/历史，机器人目标继续直接使用已选机器人的 SSH 信息；仅在对话命令需要审批时显示“运行 / 取消”。无会话兜底页也先显示相同两个入口。Stable/Beta 同步；各 54 项关联回归、四项客户端类型检查、两版完整构建、233 个共享源码对齐及 `git diff --check` 均通过。最新 Beta 源码窗口已重启，主进程 PID 8164、窗口标题 `OpenFox Beta`、响应正常。未完成：用户界面验收并提交、推送。
