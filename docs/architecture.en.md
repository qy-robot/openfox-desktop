...
The Client face accepts only the unified extended presentation. It disables the official root layout and installs its own Desktop layout/sidebar registration, which continues to host the official sidebar, conversation, and details occupants inside an inverted-L material frame. macOS and Windows apply capability-gated native materials without changing ownership of upstream occupant slots. Legacy compatibility and enhanced values migrate to extended at startup.
