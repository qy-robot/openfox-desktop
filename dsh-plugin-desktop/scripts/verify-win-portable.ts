...
export function verifyWindowsPortable(
  ...
  const portablePath = join(
    options.desktopRoot,
    'dist',
    `RoboCoding-${options.version}-x64-Portable.zip`,
  )
  ...
  const executable = entries.find(entry => entry.entryName.replaceAll('\\', '/') === 'RoboCoding.exe')
  if (executable === undefined) {
    throw new Error(`Windows portable archive is missing RoboCoding.exe: ${portablePath}`)
  }
