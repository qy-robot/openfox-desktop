...
export function verifyWindowsPortable(
  ...
  const portablePath = join(
    options.desktopRoot,
    'dist',
    `OpenFox-${options.version}-x64-Portable.zip`,
  )
  ...
  const executable = entries.find(entry => entry.entryName.replaceAll('\\', '/') === 'OpenFox.exe')
  if (executable === undefined) {
    throw new Error(`Windows portable archive is missing OpenFox.exe: ${portablePath}`)
  }
