# Extended chrome isolation

The unified extended presentation uses the isolated frame originally introduced in PR #868. Renderer crash recovery from PR #869 remains enabled.

On macOS and Windows, the extended presentation uses two documents in one native window:
...
- The client plugin does not mount a duplicate titlebar or adjust the upstream root. Global plugin CSS, inherited variables, body portals, and stacking order remain inside the content document.
...
- Version/update controls and restart/developer menus remain Desktop-owned React controls. No mode selector is rendered and no native Menu replaces these interactions.
...
The unified extended presentation uses the isolated chrome on macOS and Windows. Linux is not supported. This design does not introduce Shadow DOM.
...
3. Theme and locale updates reach chrome without plugin CSS; material transitions still restart into the extended layout.
