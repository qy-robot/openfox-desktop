...
const COPY: Record<DesktopLocale, DesktopRecoveryCopy> = {
  en: {
    ...
    dataGuideBody: 'View or change the OpenFox Home directory. A factory reset removes data from the current directory, so try other recovery options first.',
    ...
    safeModeNotificationBody: 'The app is using a temporary environment without loading existing OpenFox Home data. The next normal startup restores access to existing data and removes the temporary environment.',
    checkpoints: 'Startup checkpoints',
    checkpointsUnavailable: 'Startup checkpoints cannot be read right now.',
    rollbackBody: 'Checkpoints save configuration from successful startups. Restoring one also restores the current Profile, the shared settings.yaml file, and patches in the OpenFox Home directory.',
    ...
    configurationFilesBody: 'View or edit the current Profile configuration and shared configuration in the OpenFox Home directory. Restart the app to apply changes.',
    ...
    dataManagementBody: 'View or change the OpenFox Home directory, which stores Profiles, plugins, settings, and conversations. Changing the directory does not delete data in the previous location.',
    ...
    selectDataDirectory: 'Select an OpenFox Home directory',
    browse: 'Browse…',
    applyDataDirectory: 'Change directory and restart',
    cancelDataDirectoryChange: 'Cancel change',
    factoryReset: 'Factory reset',
    factoryResetBody: 'Move the current OpenFox Home directory to the Trash or Recycle Bin, then restart and create a new default Profile. Project files outside this directory are kept.',
    factoryResetAction: 'Reset data and restart',
    confirmDataDirectoryChange: 'Change the data directory?',
    confirmDataDirectoryChangeMessage: 'Choose a new OpenFox Home directory?',
    confirmDataDirectoryChangeBody: 'Choose an empty folder or an existing OpenFox Home directory. An empty folder will be used to create a new environment. The previous directory will be kept.',
    ...
    confirmFactoryResetMessage: 'Remove current OpenFox Home data and initialize a new environment?',
  ...
  zh: {
    ...
    dataGuideBody: '查看或更改 OpenFox Home 目录。恢复出厂设置会移除当前目录中的数据，建议先尝试其他恢复方式。',
    ...
    safeModeNotificationBody: '当前使用临时环境，未读取原有 OpenFox Home 数据。下次正常启动时将恢复使用原有数据，并清理临时环境。',
    checkpoints: '启动检查点',
    checkpointsUnavailable: '当前无法读取启动检查点。',
    rollbackBody: '检查点保存成功启动时的配置。选择一个检查点，将同时恢复当前 Profile、共享设置文件 settings.yaml 和 OpenFox Home 目录中的补丁。',
    plugins: '插件管理',
    ...
    configurationFilesBody: '查看或编辑当前 Profile 的配置与 OpenFox Home 目录中的共享配置。更改后需重启应用才能生效。',
    ...
    dataManagement: '数据管理',
    dataManagementBody: '查看或更改 OpenFox Home 目录。Profile、插件、设置和会话等数据保存在此目录中；更改目录不会删除原目录中的数据。',
    ...
    selectDataDirectory: '选择 OpenFox Home 目录',
    browse: '浏览…',
    applyDataDirectory: '更改目录并重启',
    cancelDataDirectoryChange: '取消更改',
    factoryReset: '恢复出厂设置',
    factoryResetBody: '将当前 OpenFox Home 目录移入废纸篓或回收站，然后重启并创建新的默认 Profile。此目录以外的项目文件会保留。',
    factoryResetAction: '重置数据并重启',
    confirmDataDirectoryChange: '更改数据目录？',
    confirmDataDirectoryChangeMessage: '选择新的 OpenFox Home 目录？',
    confirmDataDirectoryChangeBody: '请选择空文件夹或已有的 OpenFox Home 目录。空文件夹将用于创建新环境；原数据目录会保留。',
    ...
    confirmRestoreDefaultDirectoryBody: 'OpenFox 将改为使用当前系统的默认 OpenFox Home 目录。当前数据目录不会被删除。',
    ...
    confirmCreateDefaultDirectoryBody: 'OpenFox 将在默认路径创建一个全新的 OpenFox Home 环境并重启。当前数据目录不会被删除。',
    confirmCreateDefaultDirectoryAction: '新建并重启',
    confirmFactoryReset: '恢复 DSH Desktop 出厂设置？',
    confirmFactoryResetMessage: '移除当前 OpenFox Home 数据并重新初始化？',
    confirmFactoryResetBody: currentDirectory => `以下目录将被移入系统废纸篓或回收站：\n\n${currentDirectory}\n\n其中保存的 Profile、插件、设置、凭据、会话、附件和工作区记录都会从 OpenFox 中移除；不会删除此目录以外的项目文件。随后 OpenFox 会重启并创建干净的默认 Profile。`,
    ...
    confirmRollbackBody: capturedAt => `将立即恢复 ${capturedAt} 创建的检查点中的 Profile、共享设置文件 settings.yaml 与 OpenFox Home 目录补丁；重启后，OpenFox 将使用回滚后的配置。`,
