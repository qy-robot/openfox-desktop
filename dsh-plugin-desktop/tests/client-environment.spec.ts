...
describe('advanced desktop layout', () => {
  ...
  it('owns native caption geometry with one fixed macOS drag strip above page content', () => {
    ...
    try {
      ...
      expect(css).toContain('.dshDesktopFrame [data-sidebar-right-panel="fullscreen"] { z-index: 900; }')
      expect(css).not.toMatch(/data-rightbar-fullscreen[^{}]*\.dshDesktopRightbarSurface \{[^}]*position: fixed;/)
