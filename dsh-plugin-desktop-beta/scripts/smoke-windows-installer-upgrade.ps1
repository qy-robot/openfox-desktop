...
$taskExistingProcesses = @(Get-CimInstance Win32_Process | Where-Object {
  $_.Name -ieq 'RoboCoding Beta.exe'
})
$taskUninstallRoots = @(
...
$taskAppPath = Join-Path $taskInstallRoot 'RoboCoding Beta.exe'
$taskUninstallerPath = Join-Path $taskInstallRoot 'Uninstall RoboCoding Beta.exe'
