...
describe('RoboCoding local skills proxy', () => {
  ...
  it('projects public v2 compatibility without nested private evidence', async () => {
    const compatibility = {
      compatibilitySchemaVersion: 2,
      rules: [{ scope: { kind: 'series', seriesIds: ['qy/x-series'], privateScope: 'PRIVATE_SCOPE' }, requirements: [{ componentId: 'qy/sdk', scheme: 'semver', operator: 'exact', value: '2.5.0', extensions: { private: 'PRIVATE_EXTENSION' } }], privateRule: 'PRIVATE_RULE' }],
      exclusions: [],
      testedTargets: [{ modelId: 'qy-x1', profileId: 'standard', result: 'passed', testedAt: '2026-09-17', evidence: ['PRIVATE_EVIDENCE'], skillContentHash: 'PRIVATE_HASH' }],
      extensions: { private: 'PRIVATE_ROOT' },
    }
    const upstream = await startUpstream((_req, res) => {
      res.writeHead(200, { 'content-type': 'application/json' })
      res.end(JSON.stringify({ ...catalog, robots: catalog.robots.map(robot => ({ ...robot, seriesId: 'qy/x-series', profiles: robot.profiles.map(profile => ({ ...profile, components: { 'qy/sdk': { version: '2.5.0', scheme: 'semver', private: 'PRIVATE_COMPONENT' } } })) })), skills: catalog.skills.map(skill => ({ ...skill, compatibility })) }))
    })
    const proxy = await startProxy(ROBO_SKILLS_CATALOG_PATH, { serviceUrl: upstream.origin })
    const response = await fetch(`${proxy.origin}${ROBO_SKILLS_CATALOG_PATH}`, { headers: { origin: proxy.origin } })
    expect(response.status).toBe(200)
    const body = await response.json()
    expect(body.skills[0].compatibility).toEqual({
      compatibilitySchemaVersion: 2,
      rules: [{ scope: { kind: 'series', seriesIds: ['qy/x-series'] }, requirements: [{ componentId: 'qy/sdk', scheme: 'semver', operator: 'exact', value: '2.5.0' }] }],
      exclusions: [],
      testedTargets: [{ modelId: 'qy-x1', profileId: 'standard', result: 'passed', testedAt: '2026-09-17' }],
    })
